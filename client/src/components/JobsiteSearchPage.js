...
const JobsiteSearchPage = () => {
    ...
    const recentToMeJobsites = useSelector((state) => state.jobsites.recentJobsitesToMe);

    const recentToAllJobsites = useSelector((state) => state.jobsites.recentJobsitesToAll)
    ...
    const handleSearch = () => {
        ...
        axios.get('/api/jobsite/search', { params: { searchTerm: search } })
            .then((res) => {
    ...
    useEffect(() => {
        axios.get('/api/jobsite/recenttoall', {
            params: {
                searchQuantity: 12,
            }
        })
        ...
        axios.get('/api/jobsite/recenttome')
            .then((res) => {
    ...
    const recentToAll = () => {
        searchCategory === 'recentToAll' ? setSearchCategory('') :
            setSearchCategory('recentToAll')
        setJobsites(recentToAllJobsites)
        console.log(jobsites)
    }
    const recentToMe = () => {
        searchCategory === 'recentToMe' ? setSearchCategory('') :
            setSearchCategory('recentToMe')
        setJobsites(recentToMeJobsites)
        console.log(jobsites)
    }
    const searchResults = () => {
        searchCategory === 'searchResults' ? setSearchCategory('') :
            setSearchCategory('searchResults')
            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={12} md={12}>
                    <ToggleButtonGroup color="primary"
                        variant="solid" exclusive value={searchCategory}>
                        <Button variant="contained" onClick={recentToMe}
                            sx={{ background: searchCategory === 'recentToMe' ? 'primary' : 'lightgrey' }}
                        >recent to me</Button>
                        <Button variant="contained" onClick={recentToAll}
                            sx={{ background: searchCategory === 'recentToAll' ? 'primary' : 'lightgrey' }}
                        >recent to all</Button>
                        <Button variant="contained" onClick={searchResults}
                            sx={{ background: searchCategory === 'searchResults' ? 'primary' : 'lightgrey' }}
                        >Search Results</Button>
                    </ToggleButtonGroup>
                </Grid>
                <Grid item xs={12} sm={12} md={12}>
                    <Grid container spacing={2} sx={{ maxHeight: '65vh', overflow: 'auto'}}>
                        <Grid item xs={12} sm={12} lg={12} xl={12}>
                            <Typography variant="h4">Jobsites{jobsites ? 'yup' : 'nope'}</Typography>
                        </Grid>
